...
<body>
	...
	<input type="hidden" name="custSSN" value="<%=custSSN%>" />
				<h4>Here are the room(s) you can rent</h4>
				<ul>
					<%
					...
					<li><%=problems%></li>
					<li class="no-bullet">
                      <button id='submit-<%=i%>' type="submit" name='submit' value="<%=room.getchainName()%>--<%=room.gethotelName()%>--<%=room.getRoomNumber()%>" onclick="return confirm('Confirm Renting?');">Rent</button>
                    </li>
					<%
						}
						}
